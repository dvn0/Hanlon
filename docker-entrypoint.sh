...
then
    if [ -z "$DOCKER_HOST"]
    then
        ./hanlon_init -j '{"persist_mode": "'$PERSIST_MODE'", "hanlon_static_path": "'$HANLON_STATIC_PATH'", "hanlon_subnets": "'$HANLON_SUBNETS'", "ipmi_utility": "ipmitool", "ipmi_username": "'$IPMI_USER'", "ipmi_password": "'$IPMI_PASS'"}'
    else
